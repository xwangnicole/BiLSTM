...
class BiLSTMmodel:
    ...
    def fit_predict(self, X_train, X_test, Y_train, Y_test, cn=4, epochs=50, batch_size=32,shuffle=True):
        ...
        self.probability=model.predict(X_test_indices)
    ...
    def _rf(train, test):
        rf.fit(train[['len','Recommend_score','pred']], train['x'])
        predicted_labels = rf.predict(test[['len','Recommend_score','pred']])
        print(accuracy_score(test['x'].values, predicted_labels))
